...
sub initialise() {
	...
	if( $config{'nagios'}{'enable'} ) {
		if( ! $config{'nagios'}{'livestatus'} and ! $config{'nagios'}{'nsca'} ) {
			do_log(0, "To use the Nagios output module, you MUST specify the location of \neither the livestatus service or the\nNSCA service in the configuration file.");
			exit 2;
		}
...
#############################################################################
# NSCA Client
# These functions derived from Send NSCA Perl Client 1.14 for NSCA 2.3
# Created by Jason Lancaster <jason\@teklabs.net>
# Data routines ported with help by Dennis Morsani and Donnie Roberts from 
# Ethan Galstad's NSCA package
...
sub nsca_send($$$$) {
	...
	return if(!$config{nagios}{nsca});
	return if($nscaerr);
    $iaddr = gethostbyname($config{nagios}{nsca});
    return if(!$iaddr);
    $paddr = sockaddr_in($port, $iaddr);
    return if(!$paddr);
    $proto = getprotobyname('tcp');

	$nscaerr = 1; # stop looping
	do_log(3,"Sending status to NSCA: $host/$svc = $status");
	eval {
    	local $SIG{ALRM} = sub { die "Connection timed out"; };
    	alarm(5); # shouldn't take this long!
	    socket (SOCK, PF_INET, SOCK_STREAM, $proto) || die "Cannot create socket";
	    connect(SOCK, $paddr) || die "Cannot connect to daemon";
	    # Get 128bit xor key and 4bit timestamp.
	    read(SOCK,$xor_key,128) || die "Cannot read key from daemon";
	    read(SOCK,$timestamp, 4) || die "Cannot read timestamp from daemon";
		generate_crc32_table();
        # Build our packet.
        my $tobecrced = pack("nxx N a4 n a64 a128 a512xx",
        $packet_version, $crc, $timestamp, $status, $host, $svc, $msg);
        ...
        my $str = pack("nxx N a4 n a64 a128 a512xx",
            $packet_version, $crc, $timestamp, $status, $host, $svc, $msg);
        # Xor the sucker.
        my $string_to_send = nsca_xor($xor_key, $str);
        # Send it...
        send(SOCK,$string_to_send,0) || die "Could not send packet";
	    close(SOCK);
    	alarm 0;
	};
	...
	if($@) {
		$nscaerr = 1;
		do_log(0,"Unable to send status to NSCA($ip): $@");
	} else {
		$nscaerr = 0;
	}
...
sub send_nagios_status($$$$) {
	...
	nsca_send($host,$svc,$status,$msg) if($config{nagios}{nsca});
	return if(!$config{'nagios'}{'livestatus'}); # No livestatus so return

	if($svc) {
#		do_log(3,"Sending Nagios status [$host][$svc]=$status");
		$lscmd=("COMMAND [$t] PROCESS_SERVICE_CHECK_RESULT;$host;$svc;$status;$msg");
	} else {
#		do_log(3,"Sending Nagios status [$host]=$status");
	...
	if(!$livestatuserr and (!$livestatus or $Monitoring::Livestatus::ErrorCode)) {
		...
		if($livestatus) { $livestatus->do($lscmd); }
...
#			send_nagios_status($name,'',$mapstatus{$status},"Host status is $status");
			...
			# disk activity
			$status = 0;
			$msg = "Total disk IO ".(int($item->{'disk:usage'}*100/1024)/100)." MB/s";
			$msg .= "|disk=".($item->{'disk:usage'}*1024).";;;0; ";
			send_nagios_status($name,$servicedesc{hosts}{disk},$status,$msg);

			# net activity
			$status = 0;
			$msg = "Total network IO ".(int($item->{'net:usage'}*100/1024)/100)." MB/s";
...
#	}
	...
	if( $config{'global'}{'daemon'} ) {
		...
		# log rotation...
		if( $has_logfile ) {
		  if(($config{'logfile'}{'rotate'} eq 'y' ) or (
			($config{'logfile'}{'rotate'} eq 'd')and($lastrotate ne (localtime)[7])
			)) {
			close LOGFILE;
			...
			print LOGFILE "---- Opening logfile after rotation ("
				.$config{'logfile'}{'rotate'}.") "
				.localtime()."\n";
			select LOGFILE; $|=32; select stdout;
